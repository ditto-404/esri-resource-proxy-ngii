...
<%!
    ...
    private boolean fetchAndPassBackToClient(HttpURLConnection con, HttpServletResponse clientResponse, boolean ignoreAuthenticationErrors) throws IOException{
        if (con!=null){
            ...
            _log(Level.INFO, "Content-Type = " + con.getContentType());
			_log(Level.INFO, "Content-Length = " + con.getContentLength());
...
// ===== CUSTOM ADDITION START =====
	private String addApiKeyToUri(String uri) {

		// Replace with your own NGII Open API key.
		final String apiKey = "YOUR_NGII_API_KEY_HERE";

		if (uri != null &&
			uri.startsWith("https://map.ngii.go.kr/openapi/") &&
			!uri.contains("apikey=")) {
	...
	// ArcGIS Enterprise Portal sometimes reconstructs its own KVP GetTile request when a WMTS
	// layer is reopened from a saved web map, and has been observed sending layer=korean (an
	// NGII style name, not a layer name) instead of the real layer identifier. NGII returns
	// 200 OK with an empty body for an unrecognized layer, which renders as a blank tile with
	// no visible error. Force the correct layer identifier here so it does not matter what the
	// client sends.
	private String fixLayerParam(String uri) {
		if (uri != null && uri.startsWith("https://map.ngii.go.kr/openapi/")) {
			uri = uri.replaceAll("([?&])layer=korean(?=&|$)", "$1layer=satellite_map");
		}
		return uri;
	}
// ===== CUSTOM ADDITION END =====
...
%><%
    ...
    try {
		...
		requestUri = fixLayerParam(requestUri);
		requestUri = addApiKeyToUri(requestUri);
		...
		_log(Level.INFO, "Request URI = " + requestUri);
		_log(Level.INFO, "HTTP Status = " + con.getResponseCode());
